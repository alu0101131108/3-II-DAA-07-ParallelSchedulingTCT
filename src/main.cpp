...
int main() 
{
  ...
  std::string cpuTime;
  std::vector<int> iterations = {10, 30, 60, 100};
  std::vector<int> operation = {INTER_SWAP, INTER_INSERT};
  std::vector<int> kMaxValues = {2, 4, 6};
  std::string opString;
      ...
      for (int km = 0; km < kMaxValues.size(); km++)
        {
          for (int op = 0; op < operation.size(); op++)
          {
            ...
            framework.setScheduler(new Gnvs(iterations[it], ALL_ITERATIONS, operation[op], kMaxValues[km], false));
